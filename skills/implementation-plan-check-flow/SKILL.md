...
`07_实施计划.md` 必须包含：
**概览区：** TL;DR、总体策略、关键决策、涉及模块
**正文区：** 阶段拆分、后端任务、前端任务、数据与契约任务、测试任务、允许修改的文件范围、不允许修改的文件、依赖关系、验证命令、停止条件
**附录区：** 顺带重构受控出口、需要人工确认的问题、Req ID 覆盖映射、已读输入、引用证据、未覆盖项、下游依赖

`08_验证计划.md` 必须包含：
**概览区：** TL;DR、验证范围、当前测试框架
**正文区：** 验证命令、单元/集成/API/前端/E2E/回归用例、边界条件、浏览器验证路径（前端受影响时）、无法自动化项和人工验证步骤
